...
router.get('/', function(req, res) {
  ...
  sequelize.query(
    'select * from USERS where ID not in ( select RECV_USER_ID from INVITES WHERE PROJECT_ID = '+ req.query.pid +' AND STATUS != -1) AND ID not in (select USER_ID from ATTENDANCES where PROJECT_ID ='+req.query.pid+')',
